...
test.describe('Dashboard - Update Avatar', () => {
    ...
    test('UPDATE_AVATAR_01: Should open photo update popup', async ({ homePage, dashboardPage, page }) => {
    ...
    test('UPDATE_AVATAR_02: Should close photo update popup', async ({ homePage, dashboardPage, page }) => {
        await dashboardPage.loginAndGotoDashboard(homePage);
        await dashboardPage.openAvatarDialog();
    ...
    test('UPDATE_AVATAR_03: Should upload image in correct format', async ({ homePage, dashboardPage, page }) => {
        await dashboardPage.loginAndGotoDashboard(homePage);
        await dashboardPage.openAvatarDialog();
    ...
    test('UPDATE_AVATAR_05: Should reject non-image file format', async ({ homePage, dashboardPage, page }) => {
        ...
        await expect(page.getByText(/format|định dạng|không hợp lệ/i)).toBeVisible({ timeout: 5000 });
    });

    test('UPDATE_AVATAR_06: Should reject image exceeding file size limit', async ({ homePage, dashboardPage, page }) => {
        await dashboardPage.loginAndGotoDashboard(homePage);
        await dashboardPage.openAvatarDialog();
    ...
    test('UPDATE_AVATAR_07: Should handle rectangular image upload', async ({ homePage, dashboardPage, page }) => {
        ...
        const avatar = page.locator('img[alt*="avatar"], img[alt*="Avatar"], .ant-avatar img').first();
    ...
    test('UPDATE_AVATAR_08: Should handle multiple upload clicks', async ({ homePage, dashboardPage, page }) => {
        await dashboardPage.loginAndGotoDashboard(homePage);
        await dashboardPage.openAvatarDialog();
    ...
    test('UPDATE_AVATAR_09: Should persist avatar after page refresh', async ({ homePage, dashboardPage, page }) => {
        await dashboardPage.loginAndGotoDashboard(homePage);
