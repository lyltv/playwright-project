...
test.describe('Search', () => {
    test('SEARCH_01: Tìm kiếm theo địa điểm từ trang chủ → hiển thị kết quả đúng location', async ({ homePage, page }) => {
    ...
    test('SEARCH_02: Tìm kiếm theo địa điểm khác từ trang kết quả → chuyển sang location mới', async ({ homePage, page }) => {
    ...
    test('SEARCH_03: Mở location picker → hiển thị đầy đủ tất cả địa điểm từ API', async ({ page, api }) => {
        await page.goto('/');
    ...
    test('SEARCH_04: Chọn ngày trên trang chủ → search bar cập nhật đúng ngày đã chọn', async ({ page }) => {
        await page.goto('/');
        ...
        const dateSection = page.locator('div').filter({ hasText: /\d{2}\/\d{2}\/\d{4}/ }).first();
        await expect(dateSection).toBeVisible();
    });

    test('SEARCH_05: Chọn ngày trên trang kết quả → date picker hoạt động bình thường', async ({ homePage, page }) => {
    ...
    test('SEARCH_06: Vào room detail → booking modal giữ lại ngày, số khách đã chọn trước đó', async ({ homePage, page }) => {
        await homePage.selectLocation('hcm');
        await page.waitForURL('**/rooms/ho-chi-minh**', { timeout: 15000 });
    ...
    test('SEARCH_07: Click vào search bar → mở rộng, click ra ngoài → thu gọn (BUG)', async ({ page }) => {
    ...
    test('SEARCH_08: Áp dụng filter giá → kết quả thay đổi theo filter (BUG)', async ({ homePage, page }) => {
    ...
    test('SEARCH_09: Trang kết quả hiển thị đủ: banner location, search bar, filters, danh sách phòng', async ({ homePage, page }) => {
        await homePage.selectLocation('hcm');
        await page.waitForURL('**/rooms/ho-chi-minh**', { timeout: 15000 });

        // Banner hiển thị tên location
        await expect(page.getByText(/hồ chí minh|ho chi minh/i).first()).toBeVisible({ timeout: 10000 });
