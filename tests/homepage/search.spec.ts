...
test.describe('Search', () => {
    test('SEARCH_01: Verify search by location functions correctly on homepage', async ({ homePage, page }) => {
    ...
    test('SEARCH_02: Verify search by location functions correctly on search result pages', async ({ homePage, page }) => {
    ...
    test('SEARCH_03: Verify UI of location picker is displayed correctly', async ({ page, api }) => {
        await page.goto('/');
    ...
    test('SEARCH_05: Verify search by dates functions correctly on search result pages', async ({ homePage, page }) => {
    ...
    test('SEARCH_06: Verified that pre-selected search criteria are preserved on booking modal', async ({ homePage, page }) => {
        ...
        if (await roomLink.isVisible({ timeout: 5000 }).catch(() => false)) {
            ...
            const bookingSection = page.locator('[class*="booking"], [class*="reserve"], [class*="order"]').first();
    ...
    test('SEARCH_09: Verify search results page returns correct results', async ({ homePage, page }) => {
        await homePage.selectLocation('hcm');
        await page.waitForURL('**/rooms/ho-chi-minh**', { timeout: 15000 });
