...
- img
- img "close-circle"
- text: Chỉ cho phép dịnh dạng (jpg, jpeg, png, gif)
```
...
> 87  |             expect(page.getByText(/format|định dạng|không hợp lệ/i)).toBeVisible({ timeout: 3000 }),
      |                                                                      ^ Error: expect(locator).toBeVisible() failed
  88  | 
  89  |             dialog.getByRole('button', { name: /Upload/i }).click(),
  90  |         ]);
  91  |     });
  92  | 
