...
```ts
  ...
  78  | 
  79  |         // 1. Chờ màn hình loading biến mất
  80  |         await page.waitForTimeout(3000);
  81  | 
  82  |         // 2. Mở Modal và ĐẢM BẢO nó xuất hiện trước khi làm tiếp
  83  |         await dashboardPage.openAvatarDialog();
  84  |         const dialog = page.getByRole('dialog');
  85  |         await expect(dialog).toBeVisible({ timeout: 5000 });
  86  | 
  87  |         const fileInput = dialog.locator('input[type="file"]');
  88  | 
  89  |         // 3. Gỡ bỏ bộ lọc chặn đuôi file của Frontend
  90  |         await fileInput.evaluate((el: HTMLInputElement) => el.removeAttribute('accept'));
  91  | 
  92  |         // 4. CHÌA KHÓA: Dùng Playwright nạp file ẢO dạng .txt thẳng từ bộ nhớ
  93  |         // (Cách này ép Chromium phải nhận file vì nó hợp lệ 100% về mặt cấu trúc)
  94  |         await fileInput.setInputFiles({
  95  |             name: 'tep-tin-loi.txt',
  96  |             mimeType: 'text/plain',
  97  |             buffer: Buffer.from('Đây là nội dung file text ảo để test lỗi định dạng'),
  98  |         });
  99  | 
  100 |         // 5. Chờ 1 giây để mắt bạn (và React) nhìn thấy chữ "No file chosen" đổi thành "tep-tin-loi.txt"
  101 |         await page.waitForTimeout(1000);
  102 | 
  103 |         // 6. Kéo cò
  104 |         await dialog.getByRole('button', { name: /Upload/i }).click();
  105 | 
  106 |         // 7. Bắt lỗi đỏ với Regex
